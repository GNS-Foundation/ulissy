...
use ulissy_types::{TypedProgram, TypedStatement, TypedStatementKind, TypedExpr, TypedExprKind, Type, TypedEnumVariant, TypedObjectField, TypedInterpolatedPart, TypedConfigField, TypedComputedBody, TypedParam, TypedMatchCase};
use ulissy_parser::ast::{self, Literal, BinaryOp, UnaryOp};
...
impl CodeGenerator {
    ...
    fn generate_main(&mut self, program: &TypedProgram) -> Result<String, CodeGenError> {
        ...
        self.emitter.emit_line("#![allow(static_mut_refs, non_snake_case)]");
        ...
        self.emitter.emit_line("async fn main() -> Result<(), GnsError> {");
        self.emitter.indent();
...
"#, self.project_name)
    ...
    fn generate_statement(&mut self, stmt: &TypedStatement) -> Result<(), CodeGenError> {
        match &stmt.kind {
            ...
            TypedStatementKind::FnDecl { name, params, return_type, body } => {
                self.generate_fn_decl(name, params, return_type, body)
            }
            TypedStatementKind::ReturnStatement(expr) => {
    ...
    fn generate_let_decl(&mut self, name: &str, ty: &Type, init: &TypedExpr) -> Result<(), CodeGenError> {
        ...
        if type_annotation != "auto" {
            self.emitter.emit_line(&format!("let {}: {} = {};", name, type_annotation, final_init));
        } else {
            self.emitter.emit_line(&format!("let {} = {};", name, final_init));
        }
        Ok(())
    }
    ...
    fn generate_var_decl(&mut self, name: &str, ty: &Type, init: &Option<TypedExpr>) -> Result<(), CodeGenError> {
        ...
        if let Some(init_expr) = init {
            ...
            if type_annotation != "auto" {
                ...
                self.emitter.emit_line(&format!("let mut {} = Default::default();", name));
            }
        }
        Ok(())
    }

    fn generate_static_var(&mut self, name: &str, ty: &Type, init: &Option<TypedExpr>) -> Result<(), CodeGenError> {
        ...
        self.emitter.emit_line(&format!("static mut {}: {} = {};", name, type_annotation, init_code));
        Ok(())
    }
    ...
    fn generate_config_static(&mut self, fields: &[TypedConfigField]) -> Result<(), CodeGenError> {
        ...
        for field in fields {
            ...
            self.emitter.emit_line(&format!("pub {}: {},", field_name, rust_type));
        }
        self.emitter.dedent();
        self.emitter.emit_line("}");
        self.emitter.newline();
    ...
    fn generate_send_statement(
        ...
        self.emitter.emit_line("let mut message = gns_runtime::Message::new();");
    ...
    fn generate_enum_display_impl(
        ...
        if type_params.is_empty() {
            self.emitter.emit_line(&format!("impl std::fmt::Display for {} {{", name));
        } else {
            let params = type_params.join(", ");
            self.emitter.emit_line(&format!(
        ...
        self.emitter.indent();
        self.emitter.emit_line("fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {");
        self.emitter.indent();
    ...
    fn generate_enum_default_impl(
        ...
        if let Some(variant) = default_variant {
            ...
            self.emitter.indent();
            self.emitter.emit_line(&format!("Self::{}", to_pascal_case(&variant.name)));
            self.emitter.dedent();
            self.emitter.emit_line("}");
            self.emitter.dedent();
            self.emitter.emit_line("}");
    ...
    fn generate_type_decl(&mut self, name: &str, fields: &[(String, Type)]) -> Result<(), CodeGenError> {
        let skip_list = ["Breadcrumb", "TIT", "Epoch", "Identity", "Location", "SensorContext"];
        if skip_list.contains(&name) {
            self.emitter.emit_comment(&format!("ULissy context: type {} (using gns-runtime definition)", name));
            self.emitter.newline();
            return Ok(());
        }
        ...
        for (f_name, f_type) in fields {
            ...
            self.emitter.emit_line(&format!("pub {}: {},", snake_name, rust_type));
        }
        ...
        let params: Vec<String> = fields.iter().map(|(n, t)| {
             format!("{}: {}", to_snake_case(n), self.type_to_rust(t))
        }).collect();
        self.emitter.emit_line(&format!("pub fn new({}) -> Self {{", params.join(", ")));
        self.emitter.indent();
        ...
        for (n, t) in fields {
             ...
             self.emitter.emit_line(&format!("pub fn {}(&self) -> {} {{", snake_name, rust_type));
             self.emitter.indent();
             self.emitter.emit_line(&format!("self.{}.clone()", snake_name));
             self.emitter.dedent();
             self.emitter.emit_line("}");
        }
    ...
    fn generate_fn_decl(
        ...
        if is_recursive {
            ...
            if return_str == "()" {
                self.emitter.emit_line(&format!("async fn {}({}) -> gns_runtime::RuntimeResult<()> {{", name, params_str.join(", ")));
            } else if !return_str.starts_with("RuntimeResult") {
                self.emitter.emit_line(&format!("async fn {}({}) -> gns_runtime::RuntimeResult<{}> {{", name, params_str.join(", "), return_str));
            } else {
                self.emitter.emit_line(&format!("async fn {}({}) -> {} {{", name, params_str.join(", "), return_str));
            }
            self.emitter.indent();
        }
        ...
        if body.is_empty() && return_str != "()" {
            self.emitter.emit_line(&format!("Ok(Default::default()) // TODO: implement {}", name));
        }
    ...
    fn generate_for_statement(
        ...
        self.emitter.emit_line(&format!("for {} in {} {{", rust_item, collection_code));
        self.emitter.indent();
    ...
    fn generate_match_statement(
        ...
        for case in cases {
            ...
            if let Some(guard) = &case.guard {
                ...
                self.emitter.emit_line(&format!("{} if {} => {{", pat_code, guard_code));
            } else {
    ...
    fn pattern_to_rust(&self, pat: &ast::Pattern, subject_type: &Type) -> String {
        match pat {
            ...
            ast::Pattern::Identifier(name) => {
                 ...
                 } else if let Type::Enum { name: enum_name, .. } = subject_type {
                      format!("{}::{}", enum_name, pascal)
            ...
            ast::Pattern::Tuple(pats) => {
                let parts: Vec<String> = pats.iter().map(|p| self.pattern_to_rust(p, subject_type)).collect();
                format!("({})", parts.join(", "))
            }
        }
    }
    ...
    fn generate_after_block(
        ...
        self.emitter.emit_line(&format!("tokio::time::sleep({}).await;", delay_code));
        self.emitter.emit_line("{");
        self.emitter.indent();
        ...
        match &expr.kind {
            ...
            TypedExprKind::Identifier(name) => {
                if name.starts_with('.') {
                    ...
                    let type_name = if let Type::Enum { name: enum_name, .. } = &expr.ty {
                         enum_name.clone()
                ...
                let call = match callee_code.as_str() {
                    ...
                    "breadcrumb" => {
                        ...
                        let prev_code = match &args[2].ty {
                            ...
                            Type::Optional(inner) if matches!(**inner, Type::Breadcrumb) => format!("{}.as_ref()", prev_arg),
                            _ => format!("Some(&{})", prev_arg),
                        };
                        format!("gns_runtime::breadcrumb().cell({}).context({}).previous({})", cell, context, prev_code)
                    },
                    "computeTIT" => {
                         format!("TIT::from_public_key({}.as_bytes())", args_vec[0])
                    },
                    "log2" => {
                        let arg = if args[0].ty == Type::Int { format!("({} as f64)", args_vec[0]) } else { args_vec[0].clone() };
                        format!("({}).log2()", arg)
                    },
                    "tanh" => {
                        let arg = if args[0].ty == Type::Int { format!("({} as f64)", args_vec[0]) } else { args_vec[0].clone() };
                    ...
                    _ => {
                        ...
                        if let Some(param_types) = self.fn_signatures.get(callee_code.as_str()) {
                            for (i, (arg_code, arg_expr)) in cast_args.iter_mut().zip(args.iter()).enumerate() {
                                if i < param_types.len() {
                                    if param_types[i] == Type::Float && (arg_expr.ty == Type::Int || !matches!(arg_expr.ty, Type::Float)) {
                                        if !arg_code.contains("as f64") && !arg_code.contains(".000") {
            ...
            TypedExprKind::MethodCall { object, method, args } => {
                ...
                let obj = if should_cast {
                    if method == "chars" || method == "len" || method == "startsWith" || method == "slice" {
                ...
                match method.as_str() {
                    ...
                    "charAt" => Ok(format!("{}.chars().nth({} as usize).unwrap().to_string()", obj, args_str)),
                    ...
                    "slice" => {
                         let use_to_string = matches!(object.ty, Type::String) || matches!(object.ty, Type::Any | Type::Unknown);
                         let conversion = if use_to_string { ".to_string()" } else { ".to_vec()" };
                         
                         let args_split: Vec<&str> = args_str.split(", ").collect();
                         if args_split.len() == 1 {
                              Ok(format!("{}[{}..]{}", obj, args_split[0], conversion))
                         } else if args_split.len() == 2 {
                              Ok(format!("{}[{}..{}]{}", obj, args_split[0], args_split[1], conversion))
                         } else {
                    ...
                    "toFixed" => {
                        Ok(format!("format!(\"{{:.{}}}\", {})", args_vec[0], obj))
                    },
                    "tit" => {
                        Ok(format!("{}.tit()", obj))
                    },
...
fn stmt_contains_call(stmt: &TypedStatement, fn_name: &str) -> bool {
    match &stmt.kind {
        ...
        TypedStatementKind::VarDecl { init: Some(init), .. } => expr_contains_call(init, fn_name),
        TypedStatementKind::IfStatement { condition, then_block, else_block } => {
            expr_contains_call(condition, fn_name)
                || body_contains_call(then_block, fn_name)
                || else_block.as_ref().map_or(false, |b| body_contains_call(b, fn_name))
        }
...
fn expr_contains_call(expr: &TypedExpr, fn_name: &str) -> bool {
    match &expr.kind {
        TypedExprKind::Call { callee, args } => {
            ...
            expr_contains_call(callee, fn_name) || args.iter().any(|a| expr_contains_call(a, fn_name))
        }
        TypedExprKind::MethodCall { object, args, .. } => {
            expr_contains_call(object, fn_name) || args.iter().any(|a| expr_contains_call(a, fn_name))
        }
...
fn needs_parens(parent_op: &BinaryOp, child: &TypedExpr) -> bool {
    match &child.kind {
        TypedExprKind::Binary { op: child_op, .. } => {
